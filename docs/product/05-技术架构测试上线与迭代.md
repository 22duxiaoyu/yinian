...
- 部署：GitHub Pages；
- 线上地址：<https://22duxiaoyu.github.io/yinian/>；
...
- 项目：Supabase，区域为新加坡；
- Authentication：邮箱密码、验证邮件、会话恢复；
- Database：Postgres + RLS；
- Storage：私有桶 `action-documents`；
- Realtime：notes、documents、insights、weekly_reports；
- Edge Functions：`analyze-insights`、`generate-weekly`、`ask-action`、`delete-account`；
...
核心关系通过 `user_id` 归属用户。洞察引用以 `evidence_refs` JSON 保存记录/文档 ID，行动使用 `notes.type = task` 表示并保留来源洞察与结果字段。前端会把文档提取文本同步到 `documents.extracted_text`，AI 默认读取摘要和关键词以减少不必要的传输与模型成本。
...
| GitHub Pages | 线上页面可访问，静态资源正常 | 持续部署检查、缓存失效策略 |
...
当前版本已经具备可公开体验的前端、云端账户、用户数据隔离、私有文件、实时同步和服务端 DeepSeek 能力，可以作为作品集产品和小规模测试版本使用。
