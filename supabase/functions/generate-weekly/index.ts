...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [{ data: notes, error: notesError }, { data: documents, error: documentsError }, { data: insights, error: insightsError }] = await Promise.all([
      client.from("notes").select("id,title,content,type,mood,tags,done,created_at,updated_at").gte("updated_at", startIso).lt("updated_at", endExclusive.toISOString()).order("updated_at", { ascending: false }).limit(100),
    ...
    const report = await generateJson(
      ...
      { suggested_layout: suggestedLayout, notes: notes || [], documents: documents || [], insights: insights || [] },
