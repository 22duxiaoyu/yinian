...
const AGENT_PREFIX = "action-local-agent-v1:";
...
const agentStatusLabels = { clarifying: "澄清目标", ready: "等待确认", active: "执行中", paused: "已暂停", completed: "已完成", cancelled: "已取消" };

function emptyAgentState() {
    return { goal: null, messages: [], steps: [], checkIns: [], progress: { completed: 0, total: 0, percentage: 0 } };
}
...
const el = {
    ...
    agentOverviewCard: document.querySelector("#agentOverviewCard"),
    agentOverviewTitle: document.querySelector("#agentOverviewTitle"),
    agentOverviewCopy: document.querySelector("#agentOverviewCopy"),
    agentOverviewStatus: document.querySelector("#agentOverviewStatus"),
    agentOverviewProgress: document.querySelector("#agentOverviewProgress"),
    agentOverviewBar: document.querySelector("#agentOverviewBar"),
    agentOverviewOpen: document.querySelector("#agentOverviewOpen"),
    ...
    weeklyAgentContext: document.querySelector("#weeklyAgentContext"),
    ...
    agentNewGoal: document.querySelector("#agentNewGoal"),
    agentStageStrip: document.querySelector("#agentStageStrip"),
    agentEmpty: document.querySelector("#agentEmpty"),
    agentConversation: document.querySelector("#agentConversation"),
    agentPlanPanel: document.querySelector("#agentPlanPanel"),
    agentPlanTitle: document.querySelector("#agentPlanTitle"),
    agentPlanMeta: document.querySelector("#agentPlanMeta"),
    agentPlanSteps: document.querySelector("#agentPlanSteps"),
    agentApprove: document.querySelector("#agentApprove"),
    agentRevise: document.querySelector("#agentRevise"),
    agentPause: document.querySelector("#agentPause"),
    agentComposerLabel: document.querySelector("#agentComposerLabel"),
    agentComposerHint: document.querySelector("#agentComposerHint"),
...
function normalizeAgentState(value) {
    ...
    const goal = source.goal && typeof source.goal === "object" ? {
        ...source.goal,
        id: String(source.goal.id || createId()),
        title: String(source.goal.title || "新的行动目标"),
        objective: String(source.goal.objective || ""),
        status: agentStatusLabels[source.goal.status] ? source.goal.status : "clarifying",
        currentPlanVersion: Number(source.goal.currentPlanVersion || 0),
    } : null;
    ...
    const steps = Array.isArray(source.steps) ? source.steps.map((step, index) => ({
        ...
        version: Number(step.version || goal?.currentPlanVersion || 1),
        durationMinutes: Number(step.durationMinutes || 15),
        status: ["proposed", "approved", "in_progress", "completed", "skipped"].includes(step.status) ? step.status : "proposed",
    })) : [];
    const checkIns = Array.isArray(source.checkIns) ? source.checkIns : [];
    const completed = steps.filter((step) => step.status === "completed").length;
    const progress = { completed, total: steps.length, percentage: steps.length ? Math.round((completed / steps.length) * 100) : 0 };
    return { goal, messages, steps, checkIns, progress };
}

function loadLocalAgent(userId) {
    try {
        return normalizeAgentState(JSON.parse(localStorage.getItem(agentKey(userId)) || "{}"));
    } catch {
        return emptyAgentState();
    }
}

function saveLocalAgent() {
    if (!state.user || state.user.cloud) return;
    state.agent = normalizeAgentState(state.agent);
    localStorage.setItem(agentKey(state.user.id), JSON.stringify(state.agent));
}
...
function normalizeNote(note) {
    ...
    return {
        ...
        agentGoalId: String(source.agentGoalId || ""),
        agentStepId: String(source.agentStepId || ""),
...
async function refreshCloudWorkspace({ render = true } = {}) {
    ...
    state.agent = normalizeAgentState(workspace.agent);
...
function setActiveUser(user) {
    ...
    if (user) {
        ...
        state.agent = loadLocalAgent(user.id);
        saveNotes();
        saveDocuments();
    } else {
...
function agentCurrentSteps() {
    const goal = state.agent?.goal;
    if (!goal) return [];
    return (state.agent.steps || []).filter((step) => !goal.currentPlanVersion || step.version === goal.currentPlanVersion).sort((a, b) => a.position - b.position);
}

function recalculateAgentProgress() {
    const steps = agentCurrentSteps();
    const completed = steps.filter((step) => step.status === "completed").length;
    const progress = { completed, total: steps.length, percentage: steps.length ? Math.round((completed / steps.length) * 100) : 0 };
...
function renderAgentOverview() {
    const goal = state.agent?.goal;
    const progress = recalculateAgentProgress();
    const buttonLabel = el.agentOverviewOpen.querySelector("span");
    if (!goal) {
        el.agentOverviewCard.dataset.status = "empty";
        el.agentOverviewStatus.textContent = "尚未设定目标";
        el.agentOverviewTitle.textContent = "把一件想推进的事交给行动 Agent";
        el.agentOverviewCopy.textContent = "它会先问清目标，再给出需要你确认的计划，不会擅自创建任务。";
        el.agentOverviewProgress.textContent = "0／0";
        el.agentOverviewBar.style.width = "0%";
        buttonLabel.textContent = "设定目标";
        return;
    }
    const nextStep = agentCurrentSteps().find((step) => !["completed", "skipped"].includes(step.status));
    const copies = {
        clarifying: "还有关键信息需要你补充，回答后才会生成计划。",
        ready: `计划已经生成，共 ${progress.total} 步。确认后才会进入行动页。`,
        active: nextStep ? `当前下一步：${nextStep.title}` : "计划正在执行，等待新的结果反馈。",
        paused: "目标已暂停，现有计划和结果都已保留。",
        completed: "计划中的步骤已经完成，可以回看结果或开始下一个目标。",
        cancelled: "这个目标已经结束，可以开始一个新的目标。",
    };
    const buttonLabels = { clarifying: "继续澄清", ready: "确认计划", active: "查看进展", paused: "继续推进", completed: "查看结果", cancelled: "新建目标" };
    el.agentOverviewCard.dataset.status = goal.status;
    el.agentOverviewStatus.textContent = agentStatusLabels[goal.status] || "行动 Agent";
    el.agentOverviewTitle.textContent = goal.title;
    el.agentOverviewCopy.textContent = copies[goal.status] || goal.objective;
    el.agentOverviewProgress.textContent = `${progress.completed}／${progress.total}`;
    el.agentOverviewBar.style.width = `${progress.percentage}%`;
    buttonLabel.textContent = buttonLabels[goal.status] || "打开 Agent";
}
...
function renderAgentRail() {
    ...
    const steps = agentCurrentSteps();
    const progress = recalculateAgentProgress();
    const messages = (state.agent.messages || []).slice(-12);
    const stage = !goal || goal.status === "clarifying" ? "goal" : goal.status === "ready" ? "plan" : "execute";
    el.agentStageStrip.dataset.stage = stage;
    el.agentStageStrip.querySelectorAll("[data-agent-stage]").forEach((item) => {
        const order = { goal: 1, plan: 2, execute: 3 };
        item.classList.toggle("active", item.dataset.agentStage === stage);
        item.classList.toggle("complete", order[item.dataset.agentStage] < order[stage]);
    });
    el.agentEmpty.hidden = Boolean(goal);
    el.agentConversation.hidden = !messages.length;
    el.agentConversation.innerHTML = messages.map((message) => `
        <article class="agent-message ${message.role}">
            <span>${message.role === "user" ? "你" : "Action"}</span>
            <p>${escapeHtml(message.content).replace(/\n/g, "<br>")}</p>
            <time>${formatRelative(message.createdAt)}</time>
        </article>`).join("");
    if (messages.length) requestAnimationFrame(() => { el.agentConversation.scrollTop = el.agentConversation.scrollHeight; });

    const showPlan = Boolean(goal && steps.length && !["clarifying", "cancelled"].includes(goal.status));
    el.agentPlanPanel.hidden = !showPlan;
    if (showPlan) {
        const statusLabels = { proposed: "待确认", approved: "待开始", in_progress: "进行中", completed: "已完成", skipped: "已跳过" };
        el.agentPlanTitle.textContent = goal.title;
        el.agentPlanMeta.textContent = `${progress.completed}／${progress.total} 步`;
        el.agentPlanSteps.innerHTML = steps.map((step) => `
            <article class="agent-plan-step" data-status="${escapeHtml(step.status)}" ${step.noteId ? `data-note-id="${escapeHtml(step.noteId)}"` : ""}>
                <i>${step.status === "completed" ? `<svg><use href="#i-check" /></svg>` : step.position}</i>
                <div><strong>${escapeHtml(step.title)}</strong><p>${escapeHtml(step.detail || step.successCriteria || "留下一个可以检查的结果")}</p><small>${step.durationMinutes} 分钟${step.dueAt ? ` · ${formatAgentDate(step.dueAt)} 前` : ""}</small></div>
                <span>${statusLabels[step.status] || "计划中"}</span>
            </article>`).join("");
    }
    const proposed = steps.some((step) => step.status === "proposed");
    el.agentApprove.hidden = !goal || goal.status !== "ready" || !proposed;
    el.agentRevise.hidden = !goal || !["ready", "active", "paused"].includes(goal.status);
    el.agentPause.hidden = !goal || !["active", "paused"].includes(goal.status);
    el.agentPause.textContent = goal?.status === "paused" ? "恢复目标" : "暂停目标";
    el.agentNewGoal.hidden = !goal;

    const composer = {
        empty: ["描述你的目标", "例如：我想在两周内完成一版作品集…", "计划由你确认后才会进入行动页。"],
        clarifying: ["补充关键信息", "直接回答上面的问题…", "Action 会根据你的回答继续澄清或生成计划。"],
        ready: ["需要调整什么？", "例如：第二步太大，请拆得更小…", "现有计划不会执行，直到你点击确认。"],
        active: ["记录进展或阻碍", "例如：第一步完成了，但找不到测试用户…", "Action 会根据真实反馈调整下一步，重大变更仍需你确认。"],
        paused: ["目标已暂停", "先恢复目标，再继续记录进展。", "暂停不会删除已有计划和行动。"],
        completed: ["这个目标已经完成", "点击“新目标”开始下一件事。", "本次目标与结果会继续进入周报。"],
        cancelled: ["开始一个新目标", "点击“新目标”重新开始。", "已结束目标会保留在云端历史中。"],
    }[goal?.status || "empty"];
    el.agentComposerLabel.textContent = composer[0];
    el.aiQuestion.placeholder = composer[1];
    el.agentComposerHint.textContent = composer[2];
    el.aiQuestion.disabled = Boolean(goal && ["paused", "completed", "cancelled"].includes(goal.status));
    el.askAi.disabled = state.agentBusy || el.aiQuestion.disabled;
    el.aiStatusText.textContent = state.agentBusy ? "正在整理下一步" : goal ? `${agentStatusLabels[goal.status]} · ${progress.completed}／${progress.total}` : "等待你的目标";
}
...
function renderOverviewActions() {
    ...
    el.overviewActions.innerHTML = tasks.length ? tasks.map((note) => `
        <article class="action-row work-queue-item${note.id === state.flashTaskId ? " status-flash" : ""}" data-note-id="${escapeHtml(note.id)}">
            ...
            <div><strong>${escapeHtml(note.title)}</strong><p>${escapeHtml(note.agentStepId ? "来自 Agent 计划" : note.tags[0] ? `来自洞察：${note.tags[0]}` : "来自你的输入")}</p></div>
...
function renderBoardCards(notes, done) {
    ...
    return notes.map((note) => `
        <article class="board-card ${done ? "done" : ""}${note.id === state.flashTaskId ? " status-flash" : ""}" data-note-id="${escapeHtml(note.id)}">
            ...
            <div><strong>${escapeHtml(note.title)}</strong><p>${escapeHtml(summarize(note.resultText || note.content, 82))}</p><footer><span>${note.agentStepId ? "Action Agent" : note.resultOutcome ? escapeHtml(resultOutcomeLabels[note.resultOutcome]) : note.tags[0] ? `#${escapeHtml(note.tags[0])}` : "个人行动"}</span><time>${note.dueAt && !done ? `${formatAgentDate(note.dueAt)} 前` : formatRelative(note.completedAt || note.createdAt)}</time></footer></div>
        </article>`).join("");
}
...
function updateLocalWeeklySnapshot() {
    ...
    const agentProgress = recalculateAgentProgress();
    const agentGoal = state.agent?.goal;
    ...
    const report = {
        ...
        agent_progress: agentGoal ? {
            goal_id: agentGoal.id,
            title: agentGoal.title,
            status: agentGoal.status,
            completed_steps: agentProgress.completed,
            total_steps: agentProgress.total,
            next_check_in_at: agentGoal.nextCheckInAt || null,
        } : null,
    };
    const snapshot = {
...
function renderWeekly() {
    ...
    const storedAgentProgress = cloudReport?.agent_progress && typeof cloudReport.agent_progress === "object" ? cloudReport.agent_progress : null;
    const liveAgentGoal = !historical ? state.agent?.goal : null;
    ...
    const weeklyAgent = storedAgentProgress || (liveAgentGoal ? {
        title: liveAgentGoal.title,
        status: liveAgentGoal.status,
        completed_steps: state.agent.progress.completed,
        total_steps: state.agent.progress.total,
        next_check_in_at: liveAgentGoal.nextCheckInAt || null,
    } : null);
    el.weeklyAgentContext.hidden = !weeklyAgent;
    if (weeklyAgent) {
        const total = Number(weeklyAgent.total_steps || 0);
        const completed = Number(weeklyAgent.completed_steps || 0);
        const nextCheckIn = weeklyAgent.next_check_in_at ? ` · 下次检查 ${formatAgentDate(weeklyAgent.next_check_in_at)}` : "";
        el.weeklyAgentContext.innerHTML = `<span><svg><use href="#i-spark" /></svg>Agent 目标</span><strong>${escapeHtml(weeklyAgent.title || "本周行动目标")}</strong><em>${completed}／${total} 步 · ${escapeHtml(agentStatusLabels[weeklyAgent.status] || "已记录")}${escapeHtml(nextCheckIn)}</em>`;
    }
...
function openAiRail(opener = document.activeElement) {
    ...
    window.setTimeout(() => {
        if (!el.aiQuestion.disabled) el.aiQuestion.focus();
        else el.railClose.focus();
    }, 120);
}
...
function clearLocalWorkspace(userId) {
    [notesKey(userId), documentsKey(userId), insightsKey(userId), weeklyKey(userId), agentKey(userId), eventsKey(userId), feedbackKey(userId), `${PROFILE_PREFIX}${userId}`]
...
function toggleTask(id) {
    ...
    if (note.agentGoalId && note.agentStepId) void syncAgentStep(note);
...
function localAgentDueAt(days, hour = 18) {
    const date = new Date();
...
function buildLocalAgentPlan(goal, revision = "") {
    const version = Number(goal.currentPlanVersion || 0) + 1;
    const objective = goal.objective || goal.title;
    const adjustment = revision ? `调整要求：${revision}` : "";
    return [
        { title: "定义可验收的结果", detail: `把“${objective}”写成一个别人可以检查的结果。${adjustment}`, durationMinutes: 15, successCriteria: "写下完成标准、对象和截止范围", dueAt: localAgentDueAt(0) },
        { title: "做出第一个最小版本", detail: "只保留能验证核心判断的部分，不补装饰性内容。", durationMinutes: 45, successCriteria: "产出一个可以打开、阅读或演示的版本", dueAt: localAgentDueAt(1) },
        { title: "找一位真实对象验证", detail: "让目标用户或同学完成一次真实体验，记录理解与卡点。", durationMinutes: 30, successCriteria: "至少留下 1 条具体观察和 1 条原话", dueAt: localAgentDueAt(2) },
        { title: "根据反馈完成一次迭代", detail: "只修改最影响目标达成的问题，并保留修改前后的判断。", durationMinutes: 30, successCriteria: "完成一次有依据的修改并写下结论", dueAt: localAgentDueAt(3) },
    ].map((step, index) => ({ ...step, id: createId(), goalId: goal.id, version, position: index + 1, status: "proposed", noteId: "", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }));
}

function runLocalAgent(action, payload = {}) {
    const agent = normalizeAgentState(state.agent);
    const now = new Date().toISOString();
    const addMessage = (role, kind, content, metadata = {}) => agent.messages.push({ id: createId(), goalId: agent.goal?.id || "", role, kind, content, metadata, createdAt: now });
    let createdNotes = [];

    if (action === "start") {
        const message = String(payload.message || "").trim();
        const goalId = createId();
        agent.goal = { id: goalId, title: summarize(message, 28), objective: message, status: "clarifying", targetDate: null, successCriteria: "", constraints: {}, context: {}, currentPlanVersion: 0, nextCheckInAt: null, createdAt: now, updatedAt: now };
        agent.steps = [];
        agent.checkIns = [];
        agent.messages = [];
        addMessage("user", "goal", message);
        addMessage("assistant", "clarification", "在我替你排计划前，还需要确认两件事：\n1．什么时候算真正完成？\n2．你每天大概能投入多少时间，有什么不能碰的限制？");
    } else if (action === "answer" && agent.goal) {
        const message = String(payload.message || "").trim();
        addMessage("user", "clarification", message);
        agent.steps = buildLocalAgentPlan(agent.goal, state.agentDrafting ? message : "");
        agent.goal.status = "ready";
        agent.goal.successCriteria = message;
        agent.goal.currentPlanVersion = agent.steps[0]?.version || 1;
        agent.goal.updatedAt = now;
        addMessage("assistant", "plan", `我把目标整理成了 ${agent.steps.length} 个可验证步骤。先检查顺序和工作量；只有你确认后，它们才会进入行动页。`);
    } else if (action === "approve" && agent.goal) {
        const stepIds = new Set(agent.steps.map((step) => step.id));
        state.notes.forEach((note) => {
            if (note.agentGoalId === agent.goal.id && !note.done && !stepIds.has(note.agentStepId)) note.archived = true;
        });
        agent.steps = agent.steps.map((step, index) => {
            const existing = state.notes.find((note) => note.agentStepId === step.id);
            const note = existing || normalizeNote({ id: createId(), title: step.title, content: `${step.detail}\n\n完成标准：${step.successCriteria}`, type: "task", mood: "清醒", tags: ["Agent计划", agent.goal.title], done: false, agentGoalId: agent.goal.id, agentStepId: step.id, dueAt: step.dueAt, priority: index === 0 ? "high" : "normal", createdAt: now, updatedAt: now });
            if (!existing) createdNotes.push(note);
            return { ...step, status: "approved", noteId: note.id, updatedAt: now };
        });
        state.notes.unshift(...createdNotes);
        agent.checkIns = agent.steps.map((step) => ({ id: createId(), goalId: agent.goal.id, stepId: step.id, dueAt: step.dueAt, status: "scheduled", question: `“${step.title}”推进得怎么样？`, response: "", outcome: "" }));
        agent.goal.status = "active";
        agent.goal.nextCheckInAt = agent.checkIns[0]?.dueAt || localAgentDueAt(1);
        agent.goal.updatedAt = now;
        addMessage("assistant", "status", `计划已确认。我已把 ${agent.steps.length} 个步骤放进行动页，并会根据你的结果继续调整。`);
    } else if (action === "complete_step" && agent.goal) {
        const completed = payload.completed !== false;
        agent.steps = agent.steps.map((step) => step.id === payload.step_id ? { ...step, status: completed ? "completed" : "approved", updatedAt: now } : step);
        const allDone = agent.steps.length > 0 && agent.steps.every((step) => ["completed", "skipped"].includes(step.status));
        agent.goal.status = allDone ? "completed" : "active";
        agent.goal.nextCheckInAt = allDone ? null : agent.steps.find((step) => step.status !== "completed")?.dueAt || localAgentDueAt(1);
        agent.goal.updatedAt = now;
        if (allDone) addMessage("assistant", "status", "这个目标的计划步骤已经全部完成。下一步不是继续堆任务，而是回看结果是否真的满足了完成标准。");
    } else if (action === "check_in" && agent.goal) {
        const message = String(payload.message || "").trim();
        addMessage("user", "feedback", message);
        const needsRevision = /(目标变|计划变|重新计划|换个方向|不再需要)/.test(message);
        if (needsRevision) {
            agent.steps = buildLocalAgentPlan(agent.goal, message);
            agent.goal.currentPlanVersion = agent.steps[0].version;
            agent.goal.status = "ready";
            addMessage("assistant", "plan", "你描述的变化会影响原计划。我整理了一版新计划，旧行动暂时保留；请确认后再替换。", { replan_required: true });
        } else {
            const blocked = /(卡|难|没做|没开始|延期|来不及)/.test(message);
            agent.goal.nextCheckInAt = localAgentDueAt(2);
            addMessage("assistant", "feedback", blocked ? "收到。先不要补完整方案，把当前步骤缩成一个 15 分钟内能留下结果的动作。完成后告诉我实际卡点，我再判断是否需要改计划。" : "进展已记录。继续完成当前步骤，并把可检查的结果留在行动反馈里；我会在下一次检查时对照目标，而不是只看完成数量。");
        }
        agent.goal.updatedAt = now;
    } else if ((action === "pause" || action === "resume") && agent.goal) {
        agent.goal.status = action === "pause" ? "paused" : "active";
        agent.goal.nextCheckInAt = action === "pause" ? null : localAgentDueAt(1);
        agent.goal.updatedAt = now;
        addMessage("assistant", "status", action === "pause" ? "目标已暂停，已有计划和结果都保留。" : "目标已恢复，继续从当前未完成步骤推进。");
    } else if (action === "cancel" && agent.goal) {
        agent.goal.status = "cancelled";
        agent.goal.nextCheckInAt = null;
        agent.goal.updatedAt = now;
    }
    return { agent: normalizeAgentState(agent), createdNotes };
}

async function runAgentAction(action, payload = {}) {
    if (state.agentBusy) return null;
    state.agentBusy = true;
    renderAgentRail();
    try {
        let result;
        if (state.user?.cloud) {
            result = await window.ActionCloud.invoke("agent-orchestrate", { action, goal_id: state.agent?.goal?.id || undefined, ...payload });
            state.agent = normalizeAgentState(window.ActionCloud.mapAgent(result.agent));
            const createdNotes = (result.created_notes || []).map(window.ActionCloud.mapNote).map(normalizeNote);
            if (createdNotes.length) {
                const createdIds = new Set(createdNotes.map((note) => note.id));
                state.notes = [...createdNotes, ...state.notes.filter((note) => !createdIds.has(note.id))];
                localStorage.setItem(notesKey(state.user.id), JSON.stringify(state.notes));
            }
        } else {
            result = runLocalAgent(action, payload);
            state.agent = result.agent;
            if (result.createdNotes.length) saveNotes();
            saveLocalAgent();
            updateLocalWeeklySnapshot();
        }
        state.agentDrafting = false;
        renderAll();
        return result;
    } catch (error) {
        console.error("Action Agent failed", error);
        el.aiStatusText.textContent = "本次处理没有完成";
        showToast("Action 暂时没有完成这一步，你的数据仍然保留");
        return null;
    } finally {
...
async function submitAgentMessage() {
    const message = el.aiQuestion.value.trim();
    if (!message) return el.aiQuestion.focus();
    const goal = state.agent?.goal;
    if (goal && ["paused", "completed", "cancelled"].includes(goal.status)) {
        showToast(goal.status === "paused" ? "先恢复目标，再继续记录进展" : "请先开始一个新目标");
        return;
    }
    const action = !goal ? "start" : ["clarifying", "ready"].includes(goal.status) || state.agentDrafting ? "answer" : "check_in";
    el.aiQuestion.value = "";
    trackEvent(action === "start" ? "agent_goal_started" : action === "check_in" ? "agent_check_in_saved" : "agent_context_added", { agent_action: action });
    const result = await runAgentAction(action, { message });
    if (result && state.agent.goal?.status === "ready") trackEvent("agent_plan_generated", { step_count: state.agent.steps.length });
}

async function approveAgentPlan() {
    const result = await runAgentAction("approve");
    if (!result) return;
    trackEvent("agent_plan_approved", { step_count: state.agent.steps.length });
    showOperationFeedback("计划已进入行动页", "success");
    showToast("计划已确认，Action 会根据完成结果继续跟进");
}

function requestAgentPlanRevision() {
    state.agentDrafting = true;
    el.agentComposerLabel.textContent = "说明你想怎么调整";
    el.aiQuestion.disabled = false;
    el.aiQuestion.placeholder = "例如：第二步太大，拆成两个 20 分钟动作…";
    el.agentComposerHint.textContent = "新计划生成后仍需你再次确认。";
    el.aiQuestion.focus();
}
...
async function toggleAgentPause() {
    ...
    if (result) showToast(action === "pause" ? "目标已暂停，计划仍然保留" : "目标已恢复执行");
}

async function beginNewAgentGoal() {
    const goal = state.agent?.goal;
    if (goal && !["completed", "cancelled"].includes(goal.status) && !window.confirm("开始新目标会结束当前 Agent 目标，已有行动与结果仍会保留。继续吗？")) return;
    if (goal && !["completed", "cancelled"].includes(goal.status)) await runAgentAction("cancel");
    state.agent = emptyAgentState();
    state.agentDrafting = false;
    if (!state.user?.cloud) saveLocalAgent();
    renderAll();
    openAiRail(el.agentNewGoal);
    window.setTimeout(() => el.aiQuestion.focus(), 80);
}

async function syncAgentStep(note) {
    if (!note.agentGoalId || !note.agentStepId) return;
    const result = await runAgentAction("complete_step", { step_id: note.agentStepId, completed: note.done });
    if (result) trackEvent("agent_step_completed", { step_id: note.agentStepId, completed: note.done });
}
...
el.agentOverviewOpen.addEventListener("click", () => openAiRail(el.agentOverviewOpen));
el.railClose.addEventListener("click", () => closeAiRail());
el.aiRailBackdrop.addEventListener("click", () => closeAiRail());
el.askAi.addEventListener("click", submitAgentMessage);
el.aiQuestion.addEventListener("keydown", (event) => { if ((event.metaKey || event.ctrlKey) && event.key === "Enter") submitAgentMessage(); });
el.agentNewGoal.addEventListener("click", beginNewAgentGoal);
el.agentApprove.addEventListener("click", approveAgentPlan);
el.agentRevise.addEventListener("click", requestAgentPlanRevision);
el.agentPause.addEventListener("click", toggleAgentPause);
el.agentEmpty.addEventListener("click", (event) => {
    const prompt = event.target.closest("[data-agent-prompt]");
    if (!prompt) return;
    el.aiQuestion.value = prompt.dataset.agentPrompt || "";
    submitAgentMessage();
});
